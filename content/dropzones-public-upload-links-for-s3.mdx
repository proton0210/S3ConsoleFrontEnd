...
summary: "How to create a public upload link for an AWS S3 bucket without writing a Lambda, an API Gateway endpoint, or a custom upload portal. Buckets by ServerlessCreed Drop Zones generate a signed POST policy and a self-contained HTML upload page in one click — vendors, customers, and contractors can drag-and-drop files straight into your bucket, with size, type, and expiry limits enforced server-side by S3."
...
seo:
  ...
  description: "Create a public upload link for any S3 bucket in 30 seconds. No Lambda, no API Gateway, no custom portal. Buckets by ServerlessCreed Drop Zones generate a signed POST policy and HTML upload page with size, type, and expiry limits enforced by S3."
...
- **The policy is regenerated on demand, not stored.** Only the inputs (bucket, prefix, expiry, etc.) are persisted in encrypted local application storage. Every time you open the modal, the policy is re-signed from those inputs against the *remaining* lifetime of the zone — never a fresh 7-day window. So pausing a zone, deleting it, or letting it expire really does revoke access.
...
- **Account-scoped storage.** Drop zones are stored in encrypted local application storage, keyed by AWS account identifier. Switching to another profile in Buckets by ServerlessCreed can't surface another seat's zones — even if both seats share the same Mac user.
...
| Cost | Included in Buckets by ServerlessCreed | API Gateway + Lambda invocations | Cognito MAUs |
...
The fastest way to see this end-to-end is to install Buckets by ServerlessCreed, pick a bucket, click *Drop Zones → New zone*, and have a working public upload page in under a minute. The 14-day trial is unrestricted and doesn't ask for a credit card.

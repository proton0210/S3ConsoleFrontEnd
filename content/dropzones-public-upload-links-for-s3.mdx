...
- **Account-scoped storage.** Drop zones are stored locally in `electron-store`, keyed by AWS account identifier. Switching to another profile in Buckets by ServerlessCreed can't surface another seat's zones — even if both seats share the same Mac user.

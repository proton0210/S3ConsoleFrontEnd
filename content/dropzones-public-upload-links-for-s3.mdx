...
summary: "How to create a public upload link for an AWS S3 bucket without writing a Lambda, an API Gateway endpoint, or a custom upload portal. Serverless Buckets Drop Zones generate a signed POST policy and a self-contained HTML upload page in one click — vendors, customers, and contractors can drag-and-drop files straight into your bucket, with size, type, and expiry limits enforced server-side by S3."
author: "Serverless Buckets Team"
image: "/4-presign.png"
videoId: "jP3jpbjgXac"
keywords:
  - "S3 upload link"
...
seo:
  title: "S3 Drop Zones — Public Upload Links for S3 Buckets | Serverless Buckets"
  description: "Create a public upload link for any S3 bucket in 30 seconds. No Lambda, no API Gateway, no custom portal. Serverless Buckets Drop Zones generate a signed POST policy and HTML upload page with size, type, and expiry limits enforced by S3."
...
Serverless Buckets adds exactly one CORS rule, identified by `ID: "s3console-dropzones"`, and **merges it with whatever rules you already have**:
...
Removing a drop zone strips only its own CORS rule by ID. If that was the only rule on the bucket, Serverless Buckets runs `DeleteBucketCors` (S3 doesn't allow an empty `CORSRules` array on `PutBucketCors`). Other zones on the same bucket will stop accepting browser POSTs — that's the intended effect.
...
Step 2 matters because the policy is embedded in the page. If a vendor saved their copy of the URL, the still-valid signed policy inside the HTML would let them keep uploading — for up to 7 days — even after you deleted the zone. So Serverless Buckets tracks every hosted key under `zone.hostedKeys` and runs `DeleteObjects` on the entire set during a delete:
...
If you handed someone a downloaded `.html` file or they saved the page source from their browser, the policy in that page remains valid until its `expiration` timestamp. There is no way for Serverless Buckets — or AWS itself, short of rotating the IAM key that signed the policy — to invalidate an already-issued POST policy mid-flight. This is the same constraint that applies to every presigned URL or POST policy AWS produces. The right mental model: a drop zone is **time-limited**, not revocable in real time.
...
- **Account-scoped storage.** Drop zones are stored locally in `electron-store`, keyed by AWS account identifier. Switching to another profile in Serverless Buckets can't surface another seat's zones — even if both seats share the same Mac user.
...
| | **Serverless Buckets Drop Zones** | Custom Lambda + API Gateway | Cognito Identity Pool |
|---|---|---|---|
...
| Cost | Included in Serverless Buckets | API Gateway + Lambda invocations | Cognito MAUs |
...
The standard AWS-native answer is a [presigned POST policy](https://docs.aws.amazon.com/AmazonS3/latest/API/sigv4-post-example.html): you sign a JSON document with your credentials, and S3 will accept a multipart/form-data POST from anyone who presents the signature, as long as the upload matches every rule in the policy (bucket, key prefix, file size, content type, expiry). Building the form, the validation UI, and the hosting yourself is a small project. Serverless Buckets **Drop Zones** automate all of it — pick a bucket and prefix in the desktop app, click *New zone*, and you get a working public upload link with drag-and-drop, progress bars, and either a hosted S3 URL or a downloadable HTML file.

### Can I create a presigned upload link from the AWS S3 web console?

Not directly. The AWS web console can generate presigned URLs for **downloads** (object → reader) but does not have a UI for presigned **POST policies** (anonymous uploader → bucket). If you want a public upload link without building Lambda + API Gateway yourself, you either need a desktop client like Serverless Buckets or you script it against the AWS SDK.
...
5 GB. That's the S3 single-PUT ceiling — browser-based form POSTs cannot multipart, so the practical max per upload is 5 GB regardless of any policy. If you need bigger, use [Upload from URL](/blog/upload-from-url-to-s3) or the regular Serverless Buckets upload flow from the sender's machine.
...
The fastest way to see this end-to-end is to install Serverless Buckets, pick a bucket, click *Drop Zones → New zone*, and have a working public upload page in under a minute. The 14-day trial is unrestricted and doesn't ask for a credit card.

- [Download Serverless Buckets](/downloads) for Mac, Windows, or Linux
- [See pricing](/pricing) — $9/month, $79/year, or $149 one-time lifetime
- [How Serverless Buckets compares to other AWS S3 clients](/aws-s3-client)

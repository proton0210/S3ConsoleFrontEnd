---
title: "Why Buckets by ServerlessCreed Is ~700MB: An Honest Breakdown"
publishedAt: "2026-06-05"
lastModified: "2026-06-05"
summary: "Buckets by ServerlessCreed downloads at around 700MB. Here's an honest, no-spin breakdown of where that goes — the bundled AWS CLI (~230MB), the app framework, and the AWS SDKs — and why we ship the CLI so that browser-based AWS login just works on first launch, with no terminal and nothing to install."
author: "Buckets by ServerlessCreed Team"
image: "/1-profile.png"
keywords:
  - "Buckets by ServerlessCreed download size"
  - "Buckets by ServerlessCreed 700MB"
  - "AWS CLI v2 bundled"
  ...
  - "zero setup AWS app"
seo:
  title: "Why Buckets by ServerlessCreed Is ~700MB | An Honest Size Breakdown"
  description: "An honest breakdown of Buckets by ServerlessCreed's ~700MB download: the bundled AWS CLI (~230MB), the app framework, and the AWS SDKs — and why the bundled CLI makes browser AWS login work with no terminal and no installs."
  canonical: "https://buckets.serverlesscreed.com/blog/why-s3console-is-700mb"
---
...
- **The app framework.** Buckets by ServerlessCreed is a desktop app built on a Chromium-based framework (the same engine that renders the interface you actually use). A native, cross-platform desktop app carries that engine with it. This is most of the rest of the size, and it's true of essentially every app in this category.
- **The AWS SDKs and the app itself.** Buckets by ServerlessCreed talks to a lot of AWS — S3, STS, SSO, CloudFront, and more — so it bundles the relevant AWS SDK clients, plus our own code and assets.
...
So in Buckets by ServerlessCreed, the bundled CLI does the **login**, and the SDK does the **work**. You never open a terminal; we drive the CLI for you behind a normal sign-in button.
...
- **We don't touch your existing AWS config.** Buckets by ServerlessCreed's login runs in its own isolated sandbox (a private config directory), so it won't clobber your `~/.aws` files or fight with static keys you've already got set up. Your machine's AWS setup stays exactly as you left it.
...
| | Buckets by ServerlessCreed | Thin client (bring-your-own CLI) |
|---|---|---|
...
No. The CLI (and the Python embedded inside it) is bundled and sandboxed within Buckets by ServerlessCreed. You don't install it, update it, or put anything on your PATH — and it won't disturb any Python or AWS CLI you already have.
...
Buckets by ServerlessCreed is ~700MB mostly because it's a real cross-platform desktop app, plus a deliberate ~230MB: the official AWS CLI, bundled so that browser-based AWS sign-in — including SSO — just works on first launch, without ever opening a terminal or installing a thing.

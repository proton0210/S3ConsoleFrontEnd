...
- Storage of AWS credentials locally on your device, using the operating-system keychain through Electron safeStorage where available and a reduced-protection local encrypted fallback if secure OS storage is unavailable. Prefer AWS SSO or short-lived roles and full-disk encryption.
- Access controls and least-privilege principles for ServerlessCreed staff and workload identities.
...
11.1 **Desktop application.** Serverless Buckets is a desktop application and does not use HTTP cookies in the conventional web sense. The Software stores configuration data, preferences, AWS credentials, and License Keys in local files and operating-system-provided secure storage where available, with the reduced-protection local encrypted fallback described in Section 9.

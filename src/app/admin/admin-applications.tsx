...
export function AdminApplications({ project }: { project: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((item) => !normalizedQuery || [item.name, item.email, item.phone, item.operatingSystem, item.experience, item.motivation, item.communityRole, item.communityProofLink, item.communityDetails, item.links].some((value) => value?.toLowerCase().includes(normalizedQuery)))
      .sort((a, b) => {

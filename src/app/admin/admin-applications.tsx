...
type Application = {
  ...
  communityRole?: string; communityProofLink?: string; communityDetails?: string;
  collegeName?: string; city?: string; state?: string; branch?: string; graduationYear?: number;
...
export function AdminApplications({ project }: { project: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((item) => !normalizedQuery || [item.name, item.email, item.phone, item.collegeName, item.city, item.state, item.branch, item.graduationYear?.toString(), item.operatingSystem, item.experience, item.motivation, item.communityRole, item.communityProofLink, item.communityDetails, item.links].some((value) => value?.toLowerCase().includes(normalizedQuery)))
      .sort((a, b) => {
...
function ApplicationCard({ application, project, favourite, open, onFavourite, onToggle }: { application: Application; project: string; favourite: boolean; open: boolean; onFavourite: () => void; onToggle: () => void }) {
  ...
  return <article className={`rounded-2xl border bg-card shadow-sm transition ${favourite ? "border-amber-300/80" : ""}`}>
    <div className="flex items-start gap-3 p-4 sm:p-5"><button type="button" onClick={onFavourite} className={`mt-0.5 rounded-lg p-2 transition hover:bg-muted ${favourite ? "text-amber-500" : "text-muted-foreground"}`} aria-label={favourite ? `Remove ${application.name} from favourites` : `Add ${application.name} to favourites`} title={favourite ? "Remove from favourites" : "Add to favourites"}><Star className={`h-5 w-5 ${favourite ? "fill-current" : ""}`} /></button><button type="button" onClick={onToggle} className="min-w-0 flex-1 text-left" aria-expanded={open}><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h2 className="truncate text-lg font-semibold sm:text-xl">{application.name}</h2><p className="truncate text-sm text-primary">{application.email}</p><p className="mt-1 text-sm text-muted-foreground">{application.phone}</p>{application.collegeName && <p className="mt-1 truncate text-sm text-muted-foreground">{application.collegeName}</p>}</div><div className="flex shrink-0 items-center gap-2 text-sm text-muted-foreground"><time>{submitted}</time><ChevronDown className={`h-5 w-5 transition-transform ${open ? "rotate-180" : ""}`} /></div></div><div className="mt-3 flex flex-wrap gap-2">{application.branch && <Tag>{application.branch}</Tag>}{application.graduationYear && <Tag>Graduates {application.graduationYear}</Tag>}<Tag>{application.operatingSystem}</Tag><Tag>AWS account: {application.awsAccount ? "Yes" : "No"}</Tag><Tag>Daily: {application.dailyCommitment ? "Yes" : "No"}</Tag>{favourite && <Tag>Favourite</Tag>}</div></button></div>
    {open && <div className="border-t px-5 pb-6 sm:px-7"><h3 className="mt-5 text-sm font-semibold">Education</h3><dl className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Info label="College" value={application.collegeName || "Not collected"} /><Info label="City" value={application.city || "Not collected"} /><Info label="State" value={application.state || "Not collected"} /><Info label="Branch" value={application.branch || "Not collected"} /><Info label="Graduation year" value={application.graduationYear?.toString() || "Not collected"} /></dl><dl className="mt-6 grid gap-4 sm:grid-cols-4"><Info label="Daily commitment" value={application.dailyCommitment ? "Yes" : "No"} /><Info label="AWS account" value={application.awsAccount ? "Yes" : "No"} /><Info label="Operating system" value={application.operatingSystem} /><Info label="AWS community role" value={application.communityRole || "Not collected"} /></dl><Section title={`AWS ${project === "Buckets by ServerlessCreed" ? "S3" : "DynamoDB"} experience`} text={application.experience} /><Section title="Reason for applying" text={application.motivation} />{application.communityProofLink && <LinkSection title="Community role supporting link" href={application.communityProofLink} />}{application.communityDetails && <Section title="Other skills and achievements" text={application.communityDetails} />}<Section title="Project and social links" text={application.links} /><div className="mt-5 flex flex-wrap gap-3"><Button asChild size="sm"><a href={`mailto:${application.email}`}>Email applicant</a></Button><Button asChild size="sm" variant="outline"><a href={`tel:${application.phone.replace(/\s/g, "")}`}>Call applicant</a></Button></div></div>}
  </article>;
}
...
function LinkSection({ title, href }: { title: string; href: string }) { return <section className="mt-5"><h3 className="text-sm font-semibold">{title}</h3><a className="mt-2 block break-all text-sm text-primary underline-offset-4 hover:underline" href={href} target="_blank" rel="noreferrer">{href}</a></section>; }
